...
def format_openai_error(err: Exception) -> str:
    msg = str(err)
    low = msg.lower()
    if "invalid_api_key" in low or "incorrect api key" in low or "authentication" in low:
        return (
            ...
            "(must be a real key, not placeholder, with no quotes/spaces)."
        )
    if "model" in low and ("not found" in low or "does not exist" in low or "access" in low):
        return (
            "⚠ Model access error for gpt-4o. Your key may not have access to this model. "
            "Try a key with gpt-4o access or change model."
        )
...
def run_agent(mode, genre, min_rating, year_from, year_to, mood, sim_title, lookup_title):
    ...
    try:
        ...
        return f"<div class='empty'>{esc(format_openai_error(e))}</div>"
...
# ── UI ────────────────────────────────────────────────────────────────────────
with gr.Blocks(title="BookMuse") as demo:
    gr.HTML("""
    <div class="bm-header">
    ...
    with gr.Tabs():
        ...
        with gr.Tab("🔧 Agent"):
            ...
            a_clear.click(
                clear_outputs,
                outputs=[ask_out, agent_out],
            )

demo.launch(css=CSS)THUMB_COLS = ["thumbnail", "thumbnail_url", "image_url", "img_url", "cover"]
